...
class WebAppInterface(val act: MainActivity, val webView: WebView) {
        ...
        if (lastSeq != null) {
            for (i in lastSeq downTo 1) {
                ...
                try {
                    ...
                    waitForPivSession() // Ensure YubiKey is connected if needed
    ...
    /**
     * Blocks the thread if a YubiKey is needed for private key operations.
     * Stops when a PivSession is available (i.e. when a YubiKey is available for use).
     */
    private fun waitForPivSession(): PivSession? {
